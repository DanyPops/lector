...
export interface PythonReferenceManifest {
...
export interface PythonReferenceFixture {
	readonly root: string;
	readonly sourceRoot: string;
	dispose(): void;
}

export interface PythonReferenceGitFixture extends PythonReferenceFixture {
	readonly baselineRef: string;
	readonly changedRef: string;
}

const SOURCE_ROOT = fileURLToPath(new URL("../fixtures/python-reference", import.meta.url));
...
export function readPythonReferenceManifest(root = SOURCE_ROOT): PythonReferenceManifest {
	...
	if (
		...
		throw new Error("Invalid Python reference fixture manifest");
	}
	return {
...
export function materializePythonReferenceFixture(): PythonReferenceFixture {
	const root = mkdtempSync(join(tmpdir(), "lector-python-reference-"));
...
export function materializePythonReferenceGitFixture(): PythonReferenceGitFixture {
	const fixture = materializePythonReferenceFixture();
	const paymentPath = join(fixture.root, "contracts/payment.py");
	const purchasePath = join(fixture.root, "contracts/purchase.py");
	const initPath = join(fixture.root, "contracts/__init__.py");
	copyFileSync(join(fixture.root, "history/v1/payment.py"), paymentPath);
	...
	copyFileSync(join(fixture.root, "history/v2/payment.py"), purchasePath);
	writeFileSync(initPath, 'from .purchase import PaymentProcessor, PurchaseOrder, Receipt\n\n__all__ = ["PaymentProcessor", "PurchaseOrder", "Receipt"]\n');
	git(fixture.root, "add", "-A");
