...
import type { InstalledPythonVersionBounds } from "./installed-package-version.ts";
...
export class PythonResolutionContext {
	private bytesRead = 0;
	private entriesRead = 0;
	private diagnosticsRead = 0;
	private readonly root: string;
	readonly bounds: InstalledPythonVersionBounds;

	constructor(projectRoot: string, bounds: InstalledPythonVersionBounds) {
	...
	/** True only when the path resolves to a real, existing, in-bounds file -- callers use this to decide whether a given lockfile format is even present before attempting to read it. */
	...
	private checkNesting(value: unknown): unknown {
		if (parsedNesting(value) > this.bounds.maxManifestNesting) throw new ManifestResourceLimitExceeded("manifest-nesting");
		return value;
	}

	parseJson(text: string): unknown {
		return this.checkNesting(JSON.parse(text));
	}

	parseToml(text: string): unknown {
		return this.checkNesting(Bun.TOML.parse(text));
	}
}
