 ...
 * path a caller must never treat as a real project to auto-index.
 */
...
/**
 * The nearest enclosing git repository root starting from (and including) a given directory, or
 * undefined if none is found (e.g. /tmp scratch files, dotfiles outside any repo, or the walk
 * reaching the filesystem root without a match). Callers choose their own fallback -- see
 * workspace.resolvePath's own "git-root"/"filesystem-root" vs "given-directory" fallback.
 *
