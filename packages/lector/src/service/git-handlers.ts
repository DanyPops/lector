...
export function createGitHandlers(deps: GitHandlerDeps): GitHandlers {
	...
	return {
		async "workspace.gitStatus"(_registry, input) {
			const git = await requireGitRepository(input.workspaceId);
			return git.status();
		},
		async "workspace.gitLog"(_registry, input) {
			const git = await requireGitRepository(input.workspaceId);
			return { entries: await git.log(input.maxCount) };
		},
		async "workspace.gitDiff"(_registry, input) {
			const git = await requireGitRepository(input.workspaceId);
		 ...
		 */
		async "workspace.compareSymbolAcrossVersions"(_registry, input) {
